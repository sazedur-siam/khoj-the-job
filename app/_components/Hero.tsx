...
export function Hero({ stats: data }: { stats: JobStats }) {
  return (
    <section className="sticky top-[57px] z-20 border-b border-border bg-background">
      <div className="mx-auto flex max-w-6xl flex-wrap items-center gap-x-3 gap-y-1.5 px-4 py-2 sm:gap-x-5 sm:px-5 sm:py-2.5">
        ...
        />
        <span className="ml-auto hidden font-mono text-[10px] uppercase tracking-[0.2em] text-foreground-subtle md:inline">
...
function StatPill({
  ...
  return (
    <div className="flex items-center gap-1.5 sm:gap-2">
      ...
      />
      <span className="text-[10px] uppercase tracking-[0.14em] text-foreground-muted sm:text-[11px] sm:tracking-[0.18em]">
...
function Divider() {
  return (
    <span aria-hidden className="hidden h-3 w-px bg-border-strong md:inline-block" />
  );
}
